...
// Modification: namespace changed; StepActionType replaced with local SpikeClickType;
//               event args merged into this file and trimmed to the fields the spike uses.
using System.Runtime.InteropServices;

namespace OperationCaptureSpike.Hooks;

public enum SpikeClickType
{
    Click,
    DoubleClick,
    RightClick
    ...
    int clickCount) : EventArgs
{
    ...
    public SpikeClickType ActionType { get; } = actionType;
...
public sealed class GlobalMouseHook : IDisposable
{
    ...
    private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0 && (wParam == NativeMethods.WM_LBUTTONDOWN || wParam == NativeMethods.WM_RBUTTONDOWN))
        {
            ...
            {
                FlushPendingLeftClick();
                ClickCaptured?.Invoke(this, new ClickCapturedEventArgs(data.Pt.X, data.Pt.Y, SpikeClickType.RightClick, "right", 1));
            }
            else
            {
    ...
    private void HandleLeftClick(int x, int y)
    {
        ...
        lock (_clickSync)
        {
            ...
            if (_pendingLeftClick is { } pending && IsDoubleClick(pending, x, y, now))
            {
                ...
                doubleClick = new ClickCapturedEventArgs(x, y, SpikeClickType.DoubleClick, "left", 2);
            }
            else
            {
                _pendingLeftClickTimer?.Dispose();
    ...
    private void FlushPendingLeftClick()
    {
        ...
        lock (_clickSync)
        {
            ...
            _pendingLeftClick = null;
            click = new ClickCapturedEventArgs(pending.X, pending.Y, SpikeClickType.Click, "left", 1);
        }
